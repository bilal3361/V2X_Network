...
SENT_ALERT_LOG_PATH = PROJECT_ROOT / "data" / "amqp_sent_alert_log.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the V2X prediction engine and publish alerts through AMQP/RabbitMQ."
    )
    ...
    parser.add_argument(
        "--publish-predictions",
        action="store_true",
        help="Publish each vehicle trajectory prediction to the AMQP exchange with routing key predictions.<vehicle_id>.",
    )
    parser.add_argument(
...
def import_runtime_dependencies() -> tuple[Any, Any]:
    ...
    try:
        ...
        raise RuntimeError("pika is required for AMQP publishing. Install it with: pip install pika") from exc

    return traci, pika


def make_amqp_channel(pika: Any, args: argparse.Namespace) -> tuple[Any, Any]:
...
def alert_routing_key(risk_level: str) -> str:
    return f"alerts.{str(risk_level).lower()}"
...
def add_amqp_comparison_fields(payload: dict[str, Any]) -> dict[str, Any]:
    payload["protocol"] = "amqp"
...
def write_sent_alert_log(writer: csv.DictWriter, handle: Any, alert: dict[str, Any]) -> None:
    writer.writerow(
        {
            "protocol": alert.get("protocol", "amqp"),
...
def main() -> int:
    ...
    try:
        ...
        amqp_connection, amqp_channel = make_amqp_channel(pika, args)
    except Exception as exc:
    ...
    try:
        print(f"Publishing AMQP alerts to exchange {args.exchange} on {args.amqp_host}:{args.amqp_port}")
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            if args.publish_predictions and predictions_by_vehicle:
                for vehicle_id, predictions in predictions_by_vehicle.items():
                    prediction_payload = add_amqp_comparison_fields(
                        {
            ...
            for alert in alerts:
                ...
                add_amqp_comparison_fields(alert)
                routing_key = alert_routing_key(str(alert["risk_level"]))
                publish_json(pika, amqp_channel, args.exchange, routing_key, alert)
                write_sent_alert_log(sent_log_writer, sent_log_handle, alert)
                total_alerts += 1
                print(
                    f"AMQP alert published | risk={alert['risk_level']} | "
            ...
            if args.status_interval_steps > 0 and step % args.status_interval_steps == 0:
                print(
                    f"AMQP engine status | sim={sim_time:.1f} | step={step} | "
    ...
    print(f"AMQP alert engine finished. Steps={step}, alerts_published={total_alerts}")
    return 0
