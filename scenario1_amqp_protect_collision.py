...
import amqp_alert_engine as base_engine
...
PROTECTION_LOG_PATH = PROJECT_ROOT / "data" / "scenario1_amqp_protection_log.csv"
COLLISION_DIAGNOSIS_LOG_PATH = PROJECT_ROOT / "data" / "scenario1_amqp_collision_diagnosis_log.csv"


def parse_args() -> Any:
    return base_engine.parse_args()


def publish_status(pika: Any, channel: Any, args: Any, payload: dict[str, Any]) -> None:
    status_payload = base_engine.add_amqp_comparison_fields(payload)
    base_engine.publish_json(pika, channel, args.exchange, "status.protected", status_payload)
...
def main() -> int:
    ...
    try:
        traci, pika = base_engine.import_runtime_dependencies()
        ...
        amqp_connection, amqp_channel = base_engine.make_amqp_channel(pika, args)
    ...
    try:
        ...
        print("Starting Scenario1 protected AMQP engine.")
        ...
        print(
            "FCFS Dynamic Target Gate: AMQP publishes alerts while the same "
        ...
        publish_status(
            ...
            args,
            {
                "event_type": "scenario1_protected_amqp_started",
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            if args.publish_predictions and predictions_by_vehicle:
                for vehicle_id, predictions in predictions_by_vehicle.items():
                    prediction_payload = base_engine.add_amqp_comparison_fields(
                        {
            ...
            for alert in alerts:
                ...
                total_alerts += 1
                base_engine.add_amqp_comparison_fields(alert)
                routing_key = base_engine.alert_routing_key(str(alert["risk_level"]))
                base_engine.publish_json(pika, amqp_channel, args.exchange, routing_key, alert)
            ...
            if args.status_interval_steps > 0 and step % args.status_interval_steps == 0:
                print(
                    f"Protected AMQP status | sim={sim_time:.1f} | step={step} | "
        ...
        publish_status(
            ...
            args,
            {
                "event_type": "scenario1_protected_amqp_stopped",
                "generated_at_utc": utc_now_iso(),
                "steps": step,
        ...
        try:
            if pika is not None and amqp_channel is not None:
                publish_status(
                    pika,
                    amqp_channel,
                    args,
                    {
                        "event_type": "scenario1_protected_amqp_error",
                        "generated_at_utc": utc_now_iso(),
                        "error": str(exc),
                    },
                )
    ...
    print(
        "Scenario1 protected AMQP engine finished. "
