...
#These lines define the main folders. #Then these paths are created:
...
#These are the values given to the LSTM model.
TARGET_COLS = ["x", "y"] #. Output target of model
...
def inspect_keras_model_shape(model_path: Path = MODEL_PATH) -> dict[str, Any]: #Inspect and validate model shape
...
def export_scalers_and_metadata( #It loads the training dataset, fits scalers, validates model shape, and saves metadata.
...
def ensure_artifacts_exist( #This checks that these files exist:
    model_path: Path = MODEL_PATH,
    feature_scaler_path: Path = FEATURE_SCALER_PATH,
    target_scaler_path: Path = TARGET_SCALER_PATH,
    metadata_path: Path = MODEL_METADATA_PATH,
...
def build_model_feature_row(state: dict[str, Any]) -> dict[str, float]: #def build_model_feature_row(state: dict[str, Any]) -> dict[str, float]:
...
class TrajectoryPredictor:                                              # It loads and runs your trained LSTM model.
    def __init__(
        self,
        model_path: Path = MODEL_PATH,
        feature_scaler_path: Path = FEATURE_SCALER_PATH,
        ...
        self.model = load_model(model_path)                                # it load the best best_trajectory_model.keras
        ...
        alerts.append(
            {
                ...
                "simulation_time": round(float(simulation_time), 4),
